...
int main()
{
    ...
    using rlnc_encoder = kodo::rlnc::full_vector_encoder<fifi::binary8, kodo::enable_trace>;
    using rlnc_decoder = kodo::rlnc::full_vector_decoder<fifi::binary8, kodo::enable_trace>;
    ...
    auto outer_decoder_2 = decoder_factory.build();
    ...
    while (!outer_decoder_2->is_complete())
    {
        ...
        outer_decoder_2->read_payload(payload.data());
        
        outer_decoder_2->write_payload(payload.data());

        // Pass that packet to the decoder
        outer_decoder->read_payload(payload.data());
    }

    // The outer code is finished, so this would be the place where we
    ...
    for (uint32_t i = 0; i < symbols; ++i)
    {
        ...
        CryptoPP::StringSource ss( reinterpret_cast<const unsigned char*>( (vector) ), bytes_to_encrypt, true,
